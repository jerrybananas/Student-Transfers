...
public class AdminController {
	...
	public String updateStudentAdmin(@RequestParam("student_first_name") String student_first_name,
			...
			@RequestParam("uni1") String uni1, @RequestParam("uni2") String uni2,
			@RequestParam("same_city_siblings") int same_city_siblings,
			@RequestParam("underagedSiblings") int underagedSiblings, @RequestParam("familyIncome") int familyIncome) {
		...
		stud.setFirst_choice(uni1);
		stud.setSecond_choice(uni2);
		stud.setSame_city_siblings(same_city_siblings);
		stud.setUnderaged_siblings(underagedSiblings);
		stud.setFamily_income(familyIncome);
		List<Student> students = studentService.getStudents();
	...
	public String adminManageRoles(Model model) {
		List<Role> roles = roleService.getRoles();
		model.addAttribute("roles", roles);
		model.addAttribute("pageTitle", "Admin Manage Roles");
		return "admin-role";
	}

	@GetMapping("/roles/add")
	public String adminAddRoles(Model model) {
		List<User> users = userService.getUsers();
		model.addAttribute("users", users);
	...
	@GetMapping("/roles/{id}")
	public String getRoleBack(Model model, @PathVariable("id") int id) {
		List<Service> services = serviceService.getServices();
		for (Service eachService : services) {
			if (eachService.getService_id() == id) {
				ser = eachService;
	...
	@RequestMapping(value = "/roles/updateFinal", method = RequestMethod.POST, produces = { "application/json",
			"application/xml" })
	public String adminUpdateFinalRoles(@RequestParam("service_name") String service_name,
			@RequestParam("service_url") String service_url,
			@RequestParam("service_role_name") String service_role_name) {
		Service service = new Service();
		service.setService_name(service_name);
	...
	@RequestMapping(value = "/roles/addFinal", method = RequestMethod.POST, produces = { "application/json",
			"application/xml" })
	public String adminAddFinalRoles(@RequestParam("role") String role, @RequestParam("user_id") int user_id) {
		Service service = new Service();
		Role role1 = new Role();
		role1.setRole_name(role);
		role1.setRole_user_id(user_id);
		roleService.saveRole(role1);
		return "redirect:/admin/roles";
	}

	@GetMapping("/roles/delete/{id}")
	public String adminDeleteRole(Model model, @PathVariable("id") int id) {
